...
import rawDividendsInputs from '../../../test-data/DividendsInputs.json';
 ...
 *
 * Describing the file as a fixed tuple restores per-row types, so each test below reads its own
 * row with no casts, no optional chaining and no non-null assertions.
 */
interface ConstantAmountCase {
    test: string;
    mail: string;
    password: string;
    company: string;
...
const dividendsInputs = rawDividendsInputs as unknown as [
    ConstantAmountCase,
    ConstantAmountCase,
    ConstantAmountCase,
    InvalidAmountCase,
