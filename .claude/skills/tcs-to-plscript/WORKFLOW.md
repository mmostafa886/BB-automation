# tcs-to-plscript — Full Workflow Detail

Referenced from [SKILL.md](SKILL.md). Read the relevant section only — you don't need to read
this file top to bottom on every invocation.

## Table of contents

- [Input resolution](#input-resolution-execute-before-anything-else)
- [Execution flow — mandatory step order](#execution-flow--mandatory-step-order)
- [Project architecture](#project-architecture-mandatory--read-before-writing-any-code)
  - [Layer 1 — Locator repository](#layer-1--locator-repository-srclocatorspage-kebab-page-locatorsts)
  - [Layer 2 — Self-healing page class](#layer-2--self-healing-page-class-srcpagespage-kebab-page-self-healingts)
  - [Layer 3 — POM registration](#layer-3--pom-registration-srcpagespom-lazy-self-healingts)
  - [Layer 4 — Spec files](#layer-4--spec-files-testsgeneratedmoduletc-id-title-slugspects)
- [File naming rules](#file-naming-rules)
- [Output format](#output-format)
- [Save output](#save-output)
- [Coverage comparison](#coverage-comparison)
- [Execute & fix](#execute--fix-one-round-only)
- [Polish before PR](#polish-before-pr)
- [Create PR](#create-pr)
...
→ Template: [SCRIPTS.md#locator-repository-template](SCRIPTS.md#locator-repository-template)
...
→ Template: [SCRIPTS.md#self-healing-page-class-template](SCRIPTS.md#self-healing-page-class-template)
...
→ Template: [SCRIPTS.md#spec-file-template](SCRIPTS.md#spec-file-template)
...
1. **Navigate** to the relevant page using the MCP browser:
   ```
   mcp__playwright__browser_navigate  url: <BASE_URL><module-url>
   ```
...
2. **Reproduce the state** — if the failing element only appears after an interaction
   ...
   - `mcp__playwright__browser_click` to click buttons or links
   - `mcp__playwright__browser_fill_form` or `mcp__playwright__browser_type` to fill input fields
   - `mcp__playwright__browser_press_key` to submit with Enter or dismiss with Escape

3. **Capture the DOM** to find the correct selector:
   ```
   mcp__playwright__browser_snapshot          ← accessibility tree (preferred — reveals roles, labels, test-ids)
   mcp__playwright__browser_take_screenshot   ← visual confirmation when snapshot is ambiguous
   ```
...
Commit and open a PR using the commands in
[SCRIPTS.md#git-commit--pr-creation-commands](SCRIPTS.md#git-commit--pr-creation-commands).
